...
\end{equation}
where $E^{\star}_k$ is the latent exposure of minor group $k$ and $b^{M}_k$ the model-$M$ bias. Substituting the second expression into the first and separating the latent and bias components,
...
\end{equation}
Differencing \cref{eq:A2} across regions $r \in \{\Scot, \rUK\}$, the latent parts collapse to $\dEbar^{\star} = \Ebar^{\star}_{\Scot} - \Ebar^{\star}_{\rUK}$ and the bias parts to
...
Applying \cref{eq:A3} to two models $M$ and $M'$ and differencing the two error expressions, the latent benchmark $\dEbar^{\star}$ cancels, leaving
...
+ \frac{1}{100} \sum_k \Delta\eta_k\, b^{M}_k ,
\end{equation}
which is \cref{eq:cor2}. The intercept vanishes by \cref{eq:A4}, exactly as the mean bias does in \cref{prop:cancellation} since an affine model's additive part is a common-mode error. The slope, however, is not. It multiplies the differential, so a model that uses less of the $[0,100]$ scale returns a proportionally smaller gap in index points even where it agrees perfectly about which occupations are the exposed ones. Thus, bias $b^{M}_k$ co-varies with the exposure level and therefore, in part, with the share gap.
...
\subsection{Downstream linear functionals (\texorpdfstring{\cref{cor:functionals}}{Corollary 3})}

\begin{proof}
Recall $F^{M}_r = \sum_k \eta_{k,r}\, q_k E^{M}_k$ with coefficients $q_k \ge 0$ common
to both regions. Substituting $E^{M}_k = E^{\star}_k + b^{M}_k$ and differencing across
regions gives \cref{eq:cor3}. Unlike the unweighted case of \cref{eq:A4}, the weighted
share gaps need not sum to zero: $\sum_k \Delta\eta_k q_k = \bar{Q}_{\Scot} - \bar{Q}_{\rUK}$
with $\bar{Q}_r = \sum_k \eta_{k,r} q_k$. Under a pure level shift $b^{M}_k = b^{M}$,
\cref{eq:cor3} therefore reduces to $b^{M} \bigl(\bar{Q}_{\Scot} - \bar{Q}_{\rUK}\bigr)$. When the coefficients are constant, $q_k = q$, the sum collapses to
$q\, b^{M} \sum_k \Delta\eta_k = 0$ by \cref{eq:A4}, and the cancellation is exact.
\end{proof}

\noindent
The continuous exposure gap is the special case $q_k = 1/100$; the baseline TFP differential of \cref{sec:specs} is the special case $q_k = \kappa \pi_k$ with $\pi_k = \pi$ constant across occupations.
...
%The same factor applies to the differential, but there it multiplies a variance that \cref{prop:cancellation} has already made small, which is why the $\rho$ rows of \cref{tab:propagation} move the scoring component visibly and the composed interval hardly at all.