# LLM scoring engine. Provider-agnostic interface over httr2, with two
# transports: synchronous (small jobs, NA retries) and the Anthropic
# Batch API (full runs and calibration variants).
#
# Output schema: onet_soc_code, task_id, variant, provider, model, sub,
# comp, primary_mode, key_factors, prompt_hash, run_date.
...
# Offline mock scorer (MOCK_SCORING=1): deterministic synthetic scores
# with an occupation latent, task jitter, within-model variant noise (so
# noise variants differ and sigma^2_g > 0), and a cross-model level shift.
...
.mock_score_tasks <- function(tasks, variant, model = MODELS$M) {
  ...
  m_delta <- if (identical(model$id, MODELS$M$id)) 0 else 8
...
# Sonnet 4.6 and later reject a trailing assistant turn, so the request
# ends with the user message and the model returns the whole JSON object.
# .parse_score() isolates the {...} from any surrounding prose.
...
# Chunked synchronous scoring with checkpointing: appends each chunk to a
# checkpoint CSV and skips rows already scored, so an interrupted run
# resumes.
...
batch_submit <- function(tasks, variant, tag, model = MODELS$M) {
  ...
  if (MOCK) return(paste0("mock_", tag))
...
# Resumable batch wrapper. State machine: scores CSV exists -> read and
# return; batch ID file exists -> resume polling; otherwise submit,
# persist the ID, then poll. Delete both files to force a fresh run.
